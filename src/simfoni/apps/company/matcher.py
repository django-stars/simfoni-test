...
def match(processed_names):
    for current_name_info in processed_names:
        ...
        for name_info in processed_names:
            ...
            if all(part in name_info.parts for part in current_name_info.parts):
        ...
        for match_info in names_to_match:
            score = get_score(current_name_info.name, match_info)
            matches.append(Match(
                score=score,
                correct_order=match_info.is_correct_order,
                company=Company.objects.get(name=match_info.name_info.name),  # this place makes parsing slow. RAW?????
